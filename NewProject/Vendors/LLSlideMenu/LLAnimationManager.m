...
- (void)startAnimate{
    ...
    if (_layer.distance >= _layer.bgWidth) {
        ...
        CAKeyframeAnimation *animSpring = [self createSpringAnima:@"distance" duration:1.f usingSpringWithDamping:0.5 initialSpringVelocity:3 fromValue:@(0) toValue:@(_layer.bgWidth)];
        [_layer addAnimation:animSpring forKey:ANIM_STARTSPRING];
        CAKeyframeAnimation *animCircle = [self createBaseAnima:@"radius" duration:.4f fromValue:@(0) toValue:@(sqrt(_circleLayer.frame.size.width * _circleLayer.frame.size.width + _circleLayer.frame.size.height * _circleLayer.frame.size.width))];
        [_circleLayer addAnimation:animCircle forKey:ANIM_STARTCIRCLE];
    } else {
        CAKeyframeAnimation *animaBase = [self createBaseAnima:@"distance" duration:.4f fromValue:@(_layer.distance) toValue:@(_layer.bgWidth)];
...
- (void)endAnimate{
    ...
    if (_layer.distance >= _layer.bgWidth) {
        // 关闭背景layer
        CAKeyframeAnimation *animBase = [self createBaseAnima:@"distance" duration:.15f fromValue:@(_layer.bgWidth) toValue:@(0)];
        [_layer addAnimation:animBase forKey:ANIM_ENDBASE];
        // 关闭contentView
        CAKeyframeAnimation *animCircle = [self createBaseAnima:@"radius" duration:.15f fromValue:@(_circleLayer.frame.size.width) toValue:@(0)];
        [_circleLayer addAnimation:animCircle forKey:ANIM_ENDCIRCLE];
    }else {
        _layer.isAnimating = NO;
        CAKeyframeAnimation *animSpring = [self createSpringAnima:@"distance" duration:.75f usingSpringWithDamping:0.5 initialSpringVelocity:3 fromValue:@(_layer.distance) toValue:@(0)];
...
- (void)animationDidStop:(CAAnimation *)anim finished:(BOOL)flag {
    if (flag) {
        ...
        if (anim == [_layer animationForKey:ANIM_ENDBASE]) {
            ...
            _layer.distance = _layer.bgWidth;
            _layer.isAnimating = NO;
            CAKeyframeAnimation *animSpring = [self createSpringAnima:@"distance" duration:1.f usingSpringWithDamping:0.5 initialSpringVelocity:3 fromValue:@(_layer.bgWidth) toValue:@(0)];
